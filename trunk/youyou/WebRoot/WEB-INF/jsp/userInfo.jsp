...
-->
	...
	<body>
		...
		<div id="wrapper">
			...
			<span>昵称：<s:property value="#request.user.name" /> </span>
			<br />
			<span>邮箱:<s:property value="#request.user.email" /> </span>
			<br />
			<span>性别:<s:property value="#request.user.man" /> </span>
			<br />
			<input id="addConcern" type="button" value="加关注" />
			<a id="delConcern" href="javascript:void(0)"
				style="text-decoration: none">取消关注</a>
			<SCRIPT type="text/javascript">
				function concerned(){
					$("addConcern").value="已关注";
					$("addConcern").setAttribute("disabled","disabled");
					$("delConcern").style.visibility="visible"
				}
				function toConcern(){
				
					$("addConcern").value="加关注";
					$("addConcern").removeAttribute("disabled");
					$("delConcern").style.visibility="hidden";
				}
				if(<s:property value="#request.isConcerned"/>==true){
					concerned();
				}else{
					toConcern();
				}
				var beConcernedId=<s:property value="#request.user.id" />;
				$("addConcern").onclick=function(){
					post("IWillConcern.action","uid="+beConcernedId,function(){
				...
				$("delConcern").onclick=function(){
					post("IWillNotConcern.action","uid="+beConcernedId,function(){
						var re=eval("("+ajax.responseText+")");
						if(re.result=="success"){
							toConcern();
						}
