...
public class UserServiceImpl implements UserService {
	...
	public List<User> getAllUserIConcern(int myId) {
		List<User> userList = urd.getAllPublisherByObserverId(myId);
		for (User u : userList) {
			u.setRelation(urd.getGroupNameByObserverIdPublisherId(myId, u
					.getId()));
		}
		return userList;
	}
	...
	public List<User> getUserIConcernWithGroup(int id, int groupId) {
		int start = groupId * Util.NUM_TO_LIST_ONE_GROUP;
		int length = Util.NUM_TO_LIST_ONE_GROUP;

		List<User> userList= urd.getPublisherByObserverIdWithGroup(id, start, length);
		for (User u : userList) {
			u.setRelation(urd.getGroupNameByObserverIdPublisherId(id, u
					.getId()));
		}
		return userList;
	}

	public List<User> getUserConcernMeWithGroup(int id, int groupId) {
		int start = groupId * Util.NUM_TO_LIST_ONE_GROUP;
		int length = Util.NUM_TO_LIST_ONE_GROUP;

		return urd.getObserverByPublisherIdWithGroup(id, start, length);
	}
}
